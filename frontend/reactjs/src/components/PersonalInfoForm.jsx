...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
  ...
  const fields = [
    ...
    { key: "linkedin", label: 'LinkedIn Profiles', icon: Linkedin, type: "url" },
    { key: "website", label: 'Personal Website', icon: Globe, type: "url" },

  ];
  return (
    <div>
      <h3 className=' text-lg font-semibold to-gray-900'>Personal Information</h3>
      ...
      <div className=" flex items-center gap-2">
        <label>
          ...
          <input type="file" accept='image/jpeg,  image/png' className=' hidden' onChange={(e) => handleChange("image_url", e.target.files[0])} />
        </label>
          ...
          <div className=" flex flex-col gap-1 pl-4 text-sm">
            ...
            <label className=" relative inline-flex items-center cursor-pointer text-gray-900 gap-3">
              ...
              <input type='checkbox' className=' sr-only peer' onChange={() => {
                const nextVal = !removeBackground;
                setRemoveBackground(nextVal);
                
                if (data.image_url && typeof data.image_url === 'string') {
                  let url = data.image_url.replace(/[\?&]tr=e-bgremove/g, '');
                  if (nextVal) {
                    url += (url.includes('?') ? '&' : '?') + 'tr=e-bgremove';
                  }
